...
\newcommand{\Ho}{\ensuremath{\mathcal{H}}}
...
are use to represent the Object Logic (OL) ones; 2)
...
We are interested in using a ML to describe Dependent Type Theory (DTT) and
implement a proof search procedure based on unification.
The unification \U{} of our ML of choice Elpi~\cite{dunchev15lpar},
a dialect of $\lambda$Prolog, is able to solve higher order problems in
the pattern fragment~\cite{miller92jsc} and identifies terms up to $\eta\beta$.
Our objective is to lift \U{} to the OL retaining the same equational theory.
The problem we face is that a natural encoding of DTT (\Fo{}) makes
\U{} essentially first order. To overcome the problem we provide a better
behaved encoding \Ho{}, we show how to map \Fo{} terms to \Ho{} terms and
how to map back the unifiers found by \U, effectively implementing an higher
order unification algorithm for \Fo{}.
...
and TLC~\cite{10.1007/978-3-642-14052-5_15}. These two constitute our test bed.
\end{abstract}
