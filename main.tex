...
theorem prover, as noted in \cite{citeX, Y Z}, with the \ml serving as the
foundational infrastructure for this ecosystem. Proving a theorem, that is
...
with backtracking. One of the main expression of this proof automation is
type-class resolution\cite{wadler89, sozeau08}. Type classes are a typing
structure to introduce ad-hoc polymorphism in functional programming languages.
In \coq, the use of type class has become a more and more used programming
style. Several are the libraries based on this machinery, we can site \stdpp and
\iris.
...
\begin{itemize}
  \item about meta programming (what it is, when to use it)
  \item theorem provers (proof automation)
\end{itemize}
