...
In particular, \coq terms are encoded following the following (very simplified)
data type
...
quantification on the for \coqIn{~$\forall$~x,F} into \elpiIn{forall x\ F}.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Examples
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

Let's take the following example\footnote{taken from the \stdpp library}:
\begin{tcolorboxSimple}
  \begin{tabular}{c|c}
    \begin{minipage}[c]{0.50\linewidth}
      \begin{coqcode}
...
The \coq instance on the left is compiled in the \elpi rule on the right. If the 
query to solve (already in the \elpi syntax) is

\elpiInEq{tc-Decision (prod x\ const `nat)}

\noindent it is clear that the \elpi rule above will not unify with the query,
Since the rigid node \elpiIn{app} is different from the rigid node
\elpiIn{const}. The problem come form the lost of the fact that \coqIn{P} is an
higher-order variable in \coq, and in \elpi it is translated into a first order
variable (the variable \elpiIn{P} in \elpi does not see the binder \elpiIn{x}).

Another unification problem we can da notare che il problema dopo, is the FO
...
Specificare anche che non riusciamo a fare ne eta ne beta riduzione. 

Dire che elpi unifica in HO e con eta-beta
