...
are use to represent the object logic ones; 2)
...
that were used to implement various object logics such as
...
want to implement a unification procedure using
Elpi~\cite{dunchev15lpar}, a dialect of $\lambda$Prolog.
The unification procedure \U{} provided by Elpi solves problems in the pattern
fragment~\cite{miller92jsc} and identifies terms up to $\eta\beta$:
the equational theory that we want to lift to DTT.
The problem we face is that the natural encoding of DTT, that we call \Fo{},
makes \U{} essentially first order. To overcome the problem we provide a better
behaved encoding \Ho{}, we show how to map unification problems in \Fo{}
to related problems in \Ho{} and
how to map back the unifiers found by \U{}, effectively implementing
an higher order unification algorithm for \Fo{}.
