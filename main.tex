...
\noindent In our type-class solver, we translate the \coqIn{forall_dec} instance
...
translation lies in how the subterm \coqIn{~$\forall$~x, P x} is rendered in
the \ml rule; \coqIn{P} is a higher-order variable, and in the original subterm, it
...
(the \elpiIn{app} node), but their corresponding lists have different lengths.
...
will unify with the head of the rule for \coqIn{forall_dec}. The first
...
elpi->constr}. Secondly, the alternative encoding lacks the ability to
