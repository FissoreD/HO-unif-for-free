...
quantification on the for \coqIn{~$\forall$~x,F} into \elpiIn{prod x\ F}.
...
\noindent In our type-class solver, we compile the \coqIn{forall_dec} instance
of \coq (on the left) into the \elpi rule on the right. This compilation, which
is blindly built from the type of \coqIn{forall_dec}, hides some unification
properties accepted by the unification algorithm of the \ol but that are
rejected by the unification algorithm of the \ml. The main problem of this
compilation is how the subterm \coqIn{~$\forall$~x, P x} is translated in
the \ml rule; \coqIn{P} is a higher order variable, in the former subterm, it
does see the binder \coqIn{x}. However, the rule in the \ml, consider a first
order variable \elpiIn{P}, not seeing \elpiIn{x}. Furthermore, this subterm is
lodged within the \elpiIn{app} constructor making unification very structural.
...
\noindent The two example of queries above, already in the syntax of the \ml,
represent two terms that would unify in the \ol, but fail in the \ml. In
particular, the first fails since the \ml is not able to unify \elpiIn{const
`nat} with the term \elpiIn{app[P, x]}: they have different rigid heads. 
...
Following the definition give just before, the terms \elpiIn{app[const `f, const
`y, x]} and \elpiIn{app[P, x]} should unify with substitution $\theta := \{P
\mapsto \texttt{\elpiIn{app [const `f, const `y]}} \}$. However this unification
is not possible in the \ml since, this time we have two terms with the same head
(the \elpiIn{app} node), but the corresponding lists have different lengths.

An immediate solution to solve these unification problems\todo{mettere da qualche parte anche eta e beta}
would be to be lazy and build very general rule for each instance in the database.
This would build a rule with the following shape:
...
\noindent In this second implementation, any call to the \elpiIn{tc-Decision}
will unify with the head of the rule for \coqIn{forall_dec}. The the first
premise will ensure that the arguments unify with the what we expect from the
instance definition. Notice that, this time, the very unification algorithm of
the \ol is exploited, overcoming the unification problems listed just before.
This approach is quite appealing but it has two main disadvantages: the first
lays on the fact that the communication between the \ol and the \ml can
potentially be slow, this is due to the transformation of terms from one language
to the other. \todo{here i refer to constr->elpi e elpi->constr}
The other drawback of this latter encoding is that clause discrimination cannot
be done on the head of the clause, for instance, any indexing 
algorithm allowing to filter rules become suddenly useless. 
...
% da notare che il problema dopo, is the FO
% approx. Coq applica la regola forall\_dec sul goal (prod x\ app[f, y, x]), ma forse 
% bisognerebbe vedere se altri solver accettano questa euristica. Così facciamo 
% vedere che non siamo i soli a farlo. Nel caso contrario bisogna dire come funziona
% l'algo di unif di coq in questo caso specifico


% where \coqIn{f : A -> B -> Type} and \coqIn{y: A}, we can notice that the \elpi
% rule above cannot be applied; the terms \elpiIn{prod x\ app[P, x]} and
% \elpiIn{prod x\ app[f, y, x]} cannot unify since the lists under the
% \elpiIn{app} constructors are of different length.\todo{NOTE: the goal unify
% with forall\_dec in coq xké fa unif FO approx}

% Another example of unification failure is given by the the following instance
