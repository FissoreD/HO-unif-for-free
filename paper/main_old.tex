...
This encoding is called first order, since variables have no scope. If we take
back the subterm \coqIn{~$\forall$~x,P x} from the forall\_dec instance, we have
\coqIn{P: A ~$\to$~ Prop} and in that subterm, \coqIn{P} has in scope the
variable \coqIn{x}. The translation of this term in the ML is 
\elpiIn{all x\app[uv 1, x]}; \elpiIn{x} is not in the scope of \elpiIn{uv 1}.
...
\eqfo, is able to verify if two terms are equal up to $\eta\beta$.

The \eqho predicate to test if two terms of the 
