...
\noteE[inline]{the compiler must do beta normal!!!!}
...
\begin{enumerate}
  \item if $X = a$ or $X = y$ or $X = f \appsep a_1\ldots a_n$
    ...
    the context $\Gamma$)
    \noteE[inline]{where y comes from? $X = y$: $y$ is in the ctx of X}
...
\noteD[inline]{Should we proove simulation fidelity for \linketa insertion?}
...
terms $t$ in \maybebeta and replace them with fresh varaibles $X$.
...
\begin{proof}[Proof sketch]
  ...
  \cref{def:progBetaLL,def:progBetaRH}, then \linkbeta desappear and the
...
triggers progress \noteE{which rule?} in first \linketa that is 
removed and $E$ is assigned to $\lambda x.x$.
Under this substitution the \linkbeta becomes
\linkbetaM{x}{C_x}{(D \appsep x)}, and by \progBetaLL
it is replaced with the following two links:
\linkbetaM{x}{C_x}{F_x} and
\noteE{ma allora il link ora sparisce perhè è in llambda?}
\linketaM{}{E}{\lambda x.D_x}. The second unification
problem assignes $a$ to $B$, that in turn activates
the second \linketa ($a$ is assigned to $C$), and then all the remaining links
...
Y \mapsto a\}$.\noteE{assegnament per D incompleto}
\noteE{alla fine non si usa anche la regola beta-rigid-head?}
...
\subsection{Relaxing \cref{lemma:keep-fidelity}}
...
where the occur check does not thow an error. Note however, that the
