...
complicated since, we have a vector of lambdas, this means that the whole term
...
\begin{definition}[reduce-to]
  For any term $t$, $\lambda x_1 \ldots x_n.t_{x_0\ldots x_n}$ reduces to a bound
  variable $x$ if one of the three following cases is satisfied: 1)
  $n = 0$ and $t = x$; 2) $t$ is the application of $x$ to a list
...
\begin{definition}[occurs-rigidly]
  Given a term $t$, a bund variable ``rigidly'' occurs in \subterm{t} this
  term does not appear in the scope of a variable
\end{definition}
