...
weak-head normal form of <<\elpiIn{fapp [F|L]}>>.
...
Since $X$ is mapped to $B_x$ and $B_x$ is decompiled into $\lambda x.f \appsep x$,
...
\def\chain{\ensuremath{\mathcal{C}}\xspace}

\begin{proof}[Proof sketch]
  Let $X$ a \Fo variable appearing in two different subterms $s_1$ and $s_2$. in
  $s_1$, $X$ is applied to the list of terms $x_1\dots x_m$ and, in $s_2$, it is
  applied to $y_1\dots y_n$. Here, $m < n$. By \mapdeduplication, the
  compilation genereates two distinct \Ho variables for $X$, say $A$ and $C$,
  related by a chain \chain of \linketa{}s, with length $n - m$. We prove that
  ...
  we work with \wellb terms, this unification succeeds iff it does in \Fo. If,
  otherwise, $p < n$, then two cases should be considered: $t_1$ is a rigid
  term, then, $t_2$ must be an eta expansion of $t_1$, implying $q = 0$. If it
  is not the case, then the successive progressions made by \progressetaleft
  would eventually cause a failure. If, on the other hand, $t_1$ is a
  unification variable, \elpiIn{scope-check} guarantees that $t_2$ does not use
  ...
  decompilation is charged wake all links in \linkStore and eventually relate
  $t_1$ with $t_2$.
  %
...
\todo{update this description to match the example} 
...
$T'$ (under the context $\Gamma$).
\end{definition}
\todo{reformulate, maybe with itemize, too messy}

The idea behind this rule is to instantiate the variable of a \linketa
...
\begin{proof}[Proof sketch]
  ...
  %it makes 
  %\elpiIn{progress}
  it immediately fails and \progressetaright terminates since it performs 
  terminating operations and, if its permises succeed, the considered
  \linketa is removed from \linkStore.\todo{unclear}
%
...
property allows for a timely activation of links that grants~\fullref{prop:fidelity}.
...
hinder our exploration. We did, however, establish a
