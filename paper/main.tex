...
\newcommand{\Fo}{\texorpdfstring{\ensuremath{\mathcal{O}\xspace}}{O}} % space non va
...
that \elpiIn{forall2} applies a binary predicate to two lists (of same
...
a sequence of \emph{steps} of length $n$.
...
In \cref{sec:llam,sec:eta,sec:beta,sec:invariant1}, we will ensure
\Cref{prop:fidelity}, since we consider this property quite important. Our aim
is to guarantee that the success of \hstep coincides with the success of \fstep
at each step $p$, regardless of any newly introduced modifications in our
unification process. We postpone the discussion of \Cref{prop:fidelity-recovery}
to \cref{sec:beta}, where we delve into unifications involving terms outside
\llambda. Moreover, we reserve the proof of \Cref{prop:simulation} only in
\cref{sec:llam}, due to the absence of links. In the other sections, we omit the
proof of this latter property due to space constraints. However, the underlying
idea remain constant: link activation refines $\sigma$ so that if $\rho_i
\foUnifPb_{i_l} \Uo \sigma_i \foUnifPb_{i_r}$ then $\sigma_i \hoUnifPb_{i_l} \Ue
\sigma_i \hoUnifPb_{i_r}$. Finally, \D{\sigma_n}{\mapStore}{\linkStore_n}{\rho}
and $\rho$ is equivalent to the substitution returned by $\frun(\foUnifPb,n)$.
...
\begin{definition}[\maybeeta]\label{def:maybeeta}
  ...
  is an eta expansion. % without the lambda, 5.11 is not true, that
...
The predicate \elpiIn{scope-check} walks through all links. 
...
Since the \elpiIn{flam} node
carries no additional information (other than the function body),
each \elpiIn{abs} node can be decompiled to a \elpiIn{flam} one.

However, if the \elpiIn{flam} node was carrying more information,
...
Later, when the compiler will unsure \fullref{prop:w-enforcing}
and
...
\end{lemma}
\begin{proof}[Proof sketch]
 In this setting, \Ee is as strong as \Eo on ground terms\noteD{module var reflexivity}. What we have to show is that whenever two different \Fo{}
...
\elpiIn{con"f"} start with different, rigid, term constructors.
...
$x_1\ldots x_n$ can either be $x$ applied to terms that can
...
we know for sure if their head lambdas can be $\eta$-contracted or not.
...
\begin{proof}[Proof sketch]\todo{to reformulate}
  ...
  \begin{itemize}
    ...
    \item $s_2 \in \maybeeta$, in this case, the unification problem becomes $X \Ue Y$ 
          ...
          $X$ and $Y$, \progressetadedup is fired and, once the $\lambda$ are crossed,
...
concern these variables since they are fresh and have no corresponding
variable in \Fo.
...
with higher arity to match the lower arity. Since each \linketa can
...
combinators like \elpiIn{map} is surely possible, but 
