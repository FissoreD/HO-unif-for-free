...
%
\newcommand{\solve}{\ensuremath{\mathrm{check}}\xspace}
...
``problematic'' subterm $tp$ is replaced by a unification variable $h$
and an accessory link, that represent a suspended unification problem
$h \Ue tp$. As a result \Ue is well behaved on $t$, that is it captures \Eo.
We now define ``problematic'' formally.
...
\begin{definition}[Subterm $\mathcal{P}(t)$]
$$
\begin{array}{l}
t \in \mathcal{P}(t)\\
...
\marginpar{TODO: find better word}
\newcommand{\normal}{\ensuremath{\mathrm{normal}}}
\begin{definition}[Normal form]
Given a set of terms $X \subseteq \Ho{}$,
$$
\normal(X) =
\mathcal{P}(X) \cap (\maybebeta{} ~\cup~ \maybeeta{}) = \emptyset
$$
\end{definition}

\noindent
We write $\sigma X = \{ \sigma t | t \in X\}$.

\begin{proposition}[Normal form preservation]\label{prop:nf}
...
\Rightarrow \normal((\sigma \cup \sigma')\mathcal{T})
$$
\end{proposition}

\noindent
In particular this guarantees that is we start from normal terms
we never introduce eta-long or non-beta-normal terms in $\sigma'$.

Note that proposition \ref{prop:nf} does not hold for \Uo{} since
decompilation can introduce (actually restore) terms in
\maybeeta or \maybebeta.
...
La deduplicate eta:
  - viene chiamata che della forma [variable] -> [eta1] e [variable] -> [eta2] (a destra non c'è mai un termine con testa rigida)
  - i due termini a dx vengono unificati con la unif e uno dei due link viene buttato
    -> si mettono più lambda 
  - dopo l'unificazione rimane un link [variabile] -> [etaX] e quando
    risolviamo un eta-link, allora dobbiamo eta-espandere qb il termine di sinistra per
    poterlo  unifcare con quello di dx.
...
work in the onject language. Therefore, even if we know that $F$ is assigned
...
substitution of the meta language always contain only terms in normal form.
...
we will call $h$ and $t$ respectively the left $lhs$ and the right $rhs$ hand
side of the \linkbeta.

At compile time, a subterm is \maybebeta, if it has the shape \elpiIn{fapp[fuva
...
After its creation, a \linkbeta remain suspended until the head of $rhs$ is
instantiated by the oracle (see \cref{prop:simulation}). In this case, $rhs$ is
$\beta$-reduced to a new term, say $t'$. $t'$ is either a term in \llambda, in
...
\begin{elpicode}
  f = flam x\ flam y\ fapp[F, (X x), y] % $f = \lambda x.\lambda y.F (X x) y$
\end{elpicode}

\noindent
from the subsitution $\rho = \{X \mapsto \lambda x.x\}$.

\noindent
\otext{The following goal: \elpiIn{X = lam x\ x, app[X, a] = a}}
