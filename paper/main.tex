...
\begin{proof}[Proof sketch]\todo{to reformulate}
  ...
  \end{itemize}
  In this setting, \progressetaright does not play a critical role since \mapStore
  is a bijection; it's role is to perform variable refinement, i.e. it never fails.
...
Note \progressetaright is only needed in the next section where chains of links
are added. Hence we give an example for rule \progressetaright in the next section.
\todo{fix english}
