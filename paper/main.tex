...
node carries a term that easy to decompile since \mapStore is a bijection.
...
\begin{proof}[Proof sketch]\todo{to reformulate}
  ...
  \begin{itemize}
    \item $s_2 \notin \maybeeta$,
          in this case $s_2$ is compilet to $t_2$ and the unification step $u$ assigns $t_2$ to $X$
          ...
          $\lambda x.t_2\appsep x$, otherwise they unify thanks to \cref{clause:lam-lam}\todo{name}
          ...
          since with \cref{lam-lam}\todo{name} or \ref{clause:eta1} move under it and find
...
to ensure the follwoing property:
...
We will relax this last failure condition to accommodate for the
implementation of heuristics in~\cref{sec:heuristics}.
