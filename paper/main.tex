...
\newcommand{\Fo}{\texorpdfstring{\ensuremath{\mathcal{F}\xspace}}{F}} % space non va
...
\begin{abstract}
  ...
  proof search procedure for it using the meta language
...
The use of meta languages facilitates the implementation of
proof systems in two key ways. The first and most well-know one is that variable binding and
...
that \elpiIn{forall2} applies a binary predicate to two lists (of the same
...
\end{gather}\todo{give a name and reuse it for \Uo in next sections}
...
\begin{lemma}[Properties of \Uo in \wellb]\label{prop:uniffow} The following properties hold for \Uo
  as implemented above: \todo{reuse statement from section 2.4}
\begin{gather}
  \unifcorrectX{\Eo}{\Uo}{t}{\wellb(\{t_1, t_2\})}\\
  \unifcompleteX{\Eo}{\Uo}{t}{\wellb(\{t_1, t_2\})}
\end{gather}
\end{lemma}
\begin{proof}[Proof sketch]
...
\begin{lemma}[Properties of \Uo in $\wellb_\beta$]\label{prop:uniffowb} The following properties hold for \Uo
  where \todo{reuse 2.4}
  \begin{gather}
    \unifcorrectX{\Ee}{\Ue}{t}{\wellb_\beta(\{t_1, t_2\})}\\
    \unifcompleteX{\Ee}{\Ue}{t}{\wellb_\beta(\{t_1, t_2\})}
  \end{gather}
\end{lemma}
...
% \end{proof}
\noindent
In other word the compiler eliminates all subterms that are in \maybebeta.
...
\begin{proof}[Proof sketch]
  ...
  If $\sigma X = \lambda x.r$, then $r$ is unified with $t$: both terms are \wellb.
  Otherwise, $\lambda x. X x$ is unified with $\lambda x. t$, and $X x$ and $t$
  are both \wellb. This concludes the proof.
\end{proof}
...
\begin{proof}[Proof sketch]
  ...
  a \maybeeta, i.e. it is not a $\eta$-expansion and, so, $\wellb(\lambda x.t)$, 
...
\begin{proof}[Proof sketch]
  We want to prove that for any $\fstep_i$, $\fstep_i$ succeeds iff $\hstep_i$
  succeeds. An $\hstep_i$ is made by a unification $\mathcal{U}$ and a link
  progression $\mathcal{P}$.
  Two cases should be analyzed: 1) $\fstep_i$ succeeds, this means that $\mathcal{U}$
  succeeds since any \maybeeta term is replaced with a fresh variable, and by
  \cref{th:fidelity-beta}, the other subterms unify. Moreover, $\mathcal{P}$
  succeeds, since the $\eta$-progression
  rules simply imitate the rules~\ref{clause:eta1} and \ref{clause:eta2},
  performing unification between $\eta$-contracted terms. 
  If 2) $\fstep_i$ fails, either the failure is 
  due to the unification of terms that are not in \maybeeta. In this case
  $\mathcal{U}$ fails thanks to \cref{th:fidelity-beta}. 
  On the other hand, the failure is
  due to a unification over \maybeeta terms, making $\mathcal{P}$ to fail.
\end{proof}
...
property enables a timely activation of links that grants~\fullref{prop:fidelity}.
...
operator) \elpiIn{N} heigenvariables. Sequents are presented to
...
whose \lhs is a variable \elpiIn{X}; the second mathes and removes
a constraint on the same variable; the third
relocates \elpiIn{T2} and the latter crafs the new goal that unifies
...
\todo{mention preliminar experiment show that our prolog is 2x faster than the Coq one} 
...
We considered mechanizing our results with Abella, especially in the early phases.
Unfortunately we could not. One reason
is that eliminating the cut operator, although possible, has a high cost in
verbosity.
% he same for the \elpiIn{name}
%\todo{is it sill described?} predicate.
Another problem is that the logic of Abella does not let one quantify over
predicates and combinators like \elpiIn{forall} are used pervasively in our
code.
\todo{finish}
