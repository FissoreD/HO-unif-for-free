...
\begin{abstract}
  ...
  Notable examples of meta languages are Twelf~\cite{twelf},
  $\lambda$Prolog~\cite{miller_nadathur_2012}, Beluga~\cite{Beluga} and
...
\begin{lemma}[Properties of \Uo in \wellb]\label{prop:uniffow}
  The implementation of \Uo above is a good unification (as per \cref{prop:good-unif})
  for \Eo in the domain \wellb.
\end{lemma}
\begin{proof}[Proof sketch]
...
\newcommand{\progressetashrink}{\emph{$\eta$-progress-shrink}\xspace}
\begin{definition}[\progressetashrink]\label{def:progressetashrink}
Given a link \linketaM{\Gamma}{X}{t}, and let $S$ the scope of $X$
we pose $T = S \cap \Gamma$. If $T$ is not empty,
then the original link is replaced by a new link
\linketaM{S}{X}{t'}, where $t'$ is a term where all the variables in $\Gamma / X$ 
have been pruned.
\end{definition}
...
\begin{proof}[Proof sketch]\todo{to reformulate}
  ...
  progression $p$.
  Without loss of generality,\todo{explain why} we consider a \Fo unification problem of the form $s_1 \Uo s_2$
  where $s_1 \in \maybeeta$, which is compiled into the \linketa $\linketaM{}{X}{\lambda x.t_1}$. 
  The unification $u$ always succeeds, since $X$ is a fresh variable, we 
  only have to treat the progression $p$.
  Two cases should be analyzed:
  \begin{itemize}
    \item $s_2 \notin \maybeeta$, in this case, the unification problem becomes
          $X \Ue t_2$, which assigns $X$ to $t_2$.
          After the unification, \progressetaleft triggers the \linketa,
          and $\lambda x.t_1$ is unified with $t_2$ (or its $\eta$-expansion when needed).
          This unification succeeds iff $s_1$ and $s_2$ unify. \progressetaleft
          aims essentially to transpose the two rules \ref{clause:eta1} and \ref{clause:eta2}
          of the object language in the meta one.
    \item $s_2 \in \maybeeta$, in this case, the unification problem becomes $X \Ue Y$ 
          ...
          $X$ and $Y$, \progressetadedup is fired and $t_1$ and $t_2$ are unified.
          This unification succeeds iff $s_1 \Uo s_2$ succeeds.
  \end{itemize}
...
% 
  ...
  % and the one flam-flam we find different terms.
...
\begin{definition}[\progBetaFail]
  ...
  there exists another link $l' \in \linkStore$ with the same \lhs % as $l$;
  or the \lhs % of $l$
  become rigid.
