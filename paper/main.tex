...
$\D{\sigma}{m}{\rho}$. Links represent problematic sub-terms which
...
We represent a logic program run for a goal g as
a list steps $p$ of length $\mathcal{N}$ each made of a unification problem
between terms $\mathcal{S}_{p_l}$ and $\mathcal{S}_{p_r}$
taken from the set of all terms $\mathcal{S}$.
If a rule is used
twice in a run we make two copies of the terms (the predicates)
occurring in the head or premises. So a run is the conjunction of all
unification problems, where $\rho_0$ is the empty substitution
%
\newcommand{\solve}{\ensuremath{\mathrm{check}}}
\newcommand{\step}[4]{\ensuremath{\mathrm{step}(#1,#2,#3) \mapsto #4}}
\newcommand{\stepD}[4]{\ensuremath{#3 #1_{#2_l} \Uo #3 #1_{#2_r} \mapsto #4}}
\newcommand{\runD}[2]{\ensuremath{\bigwedge_{p = 1 \ldots #2} \step{#1}{p}{\rho_{(p-1)}}{\rho_{p}}}}
\newcommand{\run}[2]{\ensuremath{\mathrm{run}(#1,#2)}}
$$
\step{\mathcal{S}}{p}{\rho}{\rho'} := \stepD{\mathcal{S}}{p}{\rho}{\rho'}
$$
$$
\run{\mathcal{S}}{\mathcal{N}} := \runD{\mathcal{S}}{\mathcal{N}}
$$
$$
\bigwedge_{p = 1 \ldots \mathcal{N}}
\underbrace{
  \rho_{(p-1)}\mathcal{S}_{p_l} \Uo \rho_{(p-1)}\mathcal{S}_{p_l} \mapsto \rho_p
}_\mathrm{step}
$$
...
%
\begin{gather*}
\mathcal{T} \times \mathbb{M} \times \mathbb{L} = \{ (t_j,m_j,l_j) | s_j \in \mathcal{S}, \C{s_j}{t_j}{m_j}{l_j} \}\\
\bigwedge_{p = 1 \ldots \mathcal{N}}
 \underbrace{
  \sigma_{(p-1)}\mathcal{T}_{p_l} \Ue \sigma_{(p-1)}\mathcal{T}_{p_r} \mapsto \sigma_p' \land
  \solve~(\mathbb{L},\sigma_p')\mapsto \sigma_p
 }_\mathrm{step'}\\
\D{\sigma_{\mathcal{N}}}{\mathbb{L}}{\rho_{\mathcal{N}}}
\end{gather*}

\begin{proposition}[Simulation fidelity]\label{prop:fidelity}
  $\forall p, \mathrm{step}(p,\rho) \mapsto \rho' \Leftrightarrow
              \mathrm{step'}(p,\sigma) \mapsto \sigma'$
\end{proposition}

\newcommand{\maybebeta}{\ensuremath{\stackrel{?}{\beta}}\xspace}
\begin{definition}[\maybebeta]
  $t$ is in \maybebeta if it contains a subterm matching the
  pattern \elpiIn{app[uva _ _|_]}
\end{definition}

\newcommand{\maybeeta}{\ensuremath{\stackrel{?}{\eta}}\xspace}
\begin{definition}[\maybeeta]
  $t$ is in \maybeeta if it contains a subterm matching the
  pattern \elpiIn{lam x\ .. lam y\ app[con _,x, ...,uva _ [y]]}\\
   or
  \elpiIn{lam x\lam y\uva _ [y,x]}
\end{definition}

\begin{definition}[Normal form]
  We write $norm(\mathcal{T})$ when $\mathcal{T} \cap (\maybebeta \cup \maybeeta) = \emptyset$
\end{definition}

We write $\sigma\mathcal{T} = \{ \sigma t_i | t_i \in \mathcal{T}\}$.

\begin{proposition}[Normal form preservation]
  $norm(\sigma\mathcal{T}) \land \mathrm{step'}(p,\sigma) \mapsto \sigma'
  \Rightarrow norm(\sigma\mathcal{T})$   
\end{proposition}

If we focus on a single unification problem, that is
$\mathcal{S} = \{ s_1, s_2 \}$ and 
$\C{s_1}{t_1}{m_1}{l_1}$ and $\C{s_2}{t_2}{m_2}{l_2}$ we write
$$
\begin{array}{l}
s_1 \Uo s_2 \mapsto \rho \stackrel{def}{=}\\
\quad    t_1 \Ue t_2 \mapsto \sigma' \land
    \solve~(\{l_1,l_2\},\sigma') \mapsto \sigma'' \land
    \D{\sigma''}{\{l_1,l_2\}}{\rho}
\end{array}
$$
\noindent
We write $s \in \llambda$ if all unif variables in
...
Property \ref{prop:fidelity} is also relevant to use \Uo for logic programming.
In particular we want failures to occur as early as possible, so want
the check phase to take place immediately after \Ue, and fail if need be.

\paragraph{The intuition} is that a term s is compiled in a term t where any
``problematic'' sp is replaced by a hole hp and an accessory
(suspended) unification problem tp \Ue hp (called link) where
tp is the compiled of sp. 
On t \Ue is well behaved (captures \Eo), and check solves links when it can by
using \Ue or keeps them suspended until the decompilation phase.
check also detects occur checks trough the suspended unif problems
