...
\begin{definition}[\notllambda]\label{def:notllambda}
  $\notllambda = \{ X \appsep t_1 \ldots t_n ~|~ t_1 \ldots t_n ~\mathrm{are~not~distinct~names} \}$.
\end{definition}
...
\noindent
\cref{prop:nf} is key to prove \cref{prop:simulation,prop:fidelity}:
the problematic terms moved on the side by the compiler are not
put back by \hstep, hence \Ue{} can operate properly.
In \cref{sec:compilation,sec:eta,sec:beta}
we describe how the compiler recognizes terms in \maybeeta, \maybebeta and
