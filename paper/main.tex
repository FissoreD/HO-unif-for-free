...
\begin{definition}[reduce-to]
  ...
  of term $l$ and each $l_i$ reduces to $x_i$; 3) $t$ is a variable with scope
  $s$, and for any $bv \in [x|\vec{x}]$, it exists a $s_i$, such that $s_i$
  reduces to $bv$
\end{definition}

\begin{definition}[occurs-rigidly]
  A name $x$ occurs rigidly in a term $t$, if $\forall \sigma, \exists t' \in
  \subterm{t}, t' = x$, i.e. $x$ does not appear in the scope of any variable
  in $t$.
\end{definition}
...
\begin{definition}[\maybeeta detection]
  For any term $t$, $\lambda x_1 \ldots x_n.t_{x_1\ldots x_n}$ is a \maybeeta if
  1) either $t$ is the application of a term with argument $l_1 \ldots l_m$ such that 
  $m \geq n$, every $l_{m-n+i}$ reduce to $x_i$, and no $l_{1\ldots m-n-1}$
  reduces to a name in $\vec{x}$; 2) or $t$ is a variable with scope $s$ and
  each $x_i$ it exists $s_j$ such that $s_j$ reduces $x_i$.
\end{definition}
