...
\begin{proof}[Proof sketch]
  If $\sigma\hoUnifPb$ is in \llambda, then by \cref{DA FARE}, the problem 
  unifies iff its corresponding \Fo{} unifies too. If $\sigma\hoUnifPb$ is in 
  \maybebeta, then, by \cref{lemma:keep-fidelity}, the unification fails, as per
  the corresponding unification in \Fo{}.
\end{proof}

\noteD[inline]{manca una ref che dice che se \hoUnifPb is not in $\llambda$ but
$\sigma\hoUnifPb$ is in \llambda, then \hoUnifPb succeeds iff its corresponding
\Fo{} pb succeeds}.
...
Possiamo perdere OC e rimanere sospesi o commit di soluzione approx
\begin{elpicode}
