...
\printAlll
  {{{\lambda x y.X \appsep y \appsep x, \lambda x y.x},{\lambda x. f \appsep (X \appsep x) \appsep x,G}}}
  {{}}
  {{}}
  {{}}
\marginpar{TODO: remove empty T}
% \begin{gather}
...
noindent Note that here $X$ is used with different arities, moreover
...
we modify the term compiler put fresh unification variables
in their place: the problematic term is moved from 
from  $\mathbb{T}$ to \linkStore (\cref{sec:etacomp}). The compilation
of the problem $\mathbb{P}$ above is refined to: 
%
\printAlll
  {{{\lambda x.X \appsep x,f}}}
  {{{A,f}}}
...
In turn, given the map $\mathbb{M}$, this second assignment is decompiled to
...
for $s$ would break \ref{inv:uvaarity}).
