...
\begin{corollary}
  ...
  \todo[inline]{it is clear. Quello che manca è percè serve farlo, e che cosa si guadagna. Qui dimostri che fallire "ha senso" se siamo solo interessati a llambda, ma allo stesso tempo sti beta link sono lì per gestire roba fuori.}
\end{corollary}
...
\begin{proof}[Proof sketch]
  By contradiction,\todo{we assume to have a link} if $m = 0$, then the original \Fo{} term
  ...
  \linkbeta is produced: a contradiction.\todo{since we started by assuming we had a link}
\end{proof}
...
\begin{definition}[\progBetaLL]
  ...
  %\todo[inline]{Is it clearer?}
  \todo[inline]{l1 or t1? Forse è più chiaro dire che il ilnk beta viene raffinato con link beta cone meno argomenti, Se poi il numero di argomenti extra è 0 il tutto è in llambda e quindi viene buttato via. In questo modo la terminazione è ancora più chiara perchè si vede già che prima decresce la lista di argomenti e poi il numero di beta}
\end{definition}
...
\begin{proof}[Proof sketch]
  ...
  %\todo[inline]{is it ok?}
  \todo[inline]{funziona. per essere più precisi io parlerei di ordine lessicografico (tipico ordine ben fondato usato per dimostrare terminazione). Nl nostro caso è la tripla (argomenti extra dei beta, numero di beta, numero di eta).}
\end{proof}
...
triggers progress \todo{which rule?} in first \linketa that is 
...
\todo{ma allora il link ora sparisce perhè è in llambda?}
...
Y \mapsto a\}$.\todo{assegnament per D incompleto}
\todo{alla fine non si usa anche la regola beta-rigid-head?}
