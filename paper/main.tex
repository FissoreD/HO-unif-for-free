...
consider that the bound variable \elpiIn{x} does not ``rigidly'' occur in 
the given subterm.
...
variable and the \rhs is a term starting with the \elpiIn{lam} constructor. We
can also note that for any name $x$, $tx = t' \appsep x$ never start with
\elpiIn{lam}, since by contradiction, if 1) $tx$ is an $\eta$-expansion, then
the recursive call to \elpiIn{comp} would have replaced $tx$ with a fresh
...
\elpiIn{progress1} predicate. After passing under all the \elpiIn{abs}
...
of $t$) with $t'$. Finally, if \lhs starts has \elpiIn{lam} as constructor, then
...
We can note that the insertion of these rules for \elpiIn{progress1} do not
prevent the termination of \elpiIn{progress}, since: if a link is activated it
is removed from \linkStore and the recursive call to \elpiIn{progress} will have
...
suspended, then the condition of the \elpiIn{if}-statement of \elpiIn{progress}
...
relations. Let \mapStore be the list of mapping and let $m \in \mapStore$, such
that $m = \mapping{A}{\textbf{A}}{n}$, we define $\cv, \ev, ar$ such that
$\cv(m) = A$, $\ev(m) = \textbf{A}$ and $\ar(m) = n$. Let $\langle m_1, m_2
\rangle \in \mapStore$ such that $\ar(m_1) < \ar(m_2)$ and $n = ar(m_1) -
ar(m_2) - 1$. We also let $A^i$ be a fresh \Ho variable. We define the
\elpiIn{make-eta-link} relation taking two mappings $\langle m_1, m_2 \rangle$
and returning  \linketa such that:

$$ 
  \forall i \in [0 \dots n], \left\{ 
    \begin{array}{ll}
      \linketaM{\phantom{\Gamma:x_0\dots x_{i-1}}}
              {\ev(m_1)}
              {\lambda x.A^0_x} & if~ i = 0 \\
      \linketaM{\Gamma:x_0\dots x_{i-1}}
               {A^i_{\Gamma}}
               {\lambda x.A^{i+1}_{\Gamma\cup x}} & if~ 0 < i < n\\
      \linketaM{\Gamma:x_0\dots x_{i}\phantom{_{-1}}}
               {A^{i-1}_{\Gamma}}
               {\lambda x.\ev(m_2)_{\Gamma\cup x}} & if~ i = n\\
    \end{array} 
  \right. 
$$
...
linked to a fresh variable eta-expended variable $A^0$ having the scope of $v$.
This variable $A^0$ is then linked to a fresh variable $A^{1}$ with same scope
of $A^0$ and so on. The last link is built between the $A^{n-1}$ (where $n$ is
the difference of arities between the two mappings) and the \Ho variable $u$
with higher arity in the two mappings being considered.

\begin{definition}[map-deduplication]
  Forall mappings $\langle m_1, m_2 \rangle \in \mapStore$, such that $\cv(m_1) =
  \cv(m_2)$, the list of \linketa $L$ is created thanks to
...
\begin{description}
  \item[Unif problems:] $X = \lambda x.\lambda y.x, Z = f\appsep(\lambda x.x)$
  \item[Mappings:] $\mapping{F}{H}{2}$  
  \item[Links:] $l_1 = \linketaM{x}{Y_{x}}{\lambda y.H_{y~x}}$,\\
                $l_2 = \linketaM{}{X}{\lambda x.Y_{x}}$,\\
                $l_3 = \linketaM{}{Z}{\lambda x.f \appsep G_{x}~x}$\\
                $l_4 = \linketaM{x}{G_{x}}{\lambda y.H_{x~y}}$
\end{description}
...
As last step, we can note that $G$
