...
\noindent
Whenever \elpiIn{flam F} is detected to be in
...
breaks \fullref{prop:fidelity} since two incompatible assignements to
...
\begin{proposition}[\mapStore is a bijection]
  After compilation, for each \Fo{} variable $X$ in \foUnifPb{} and for each \Ho{}
  variable $A$ in \hoUnifPb{} there is exactly one entry $X \mapsto A^n$
  (for any arity $n$).
\end{proposition}

\noindent
Note that the execution in \Ho{} may require allocating new variables as
explained in~\cref{sec:decompilation}. The property above does not
concern these.

The core procedure of this post-processing step is \emph{align-arity} that
is iterated by \elpiIn{map-deduplication}:
...
\todo{verificare, la vecchia soluzione era sballata}
...
\noindent Note that the shape of such as \rhs is
  <<\elpiIn{app [uva X S | L]}>>, where \elpiIn{S}~$= s_1\dots s_n$
...
\noindent
The list \elpiIn{Ag} is split into the two parts: 
...
The variable \elpiIn{B}, returned has the compiled term, is a fresh variable
