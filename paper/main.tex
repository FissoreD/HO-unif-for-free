...
Unification variables standing for functions are applied to variables
...
\marginpar{maybe move code to the appendix and keep only signatures}
\begin{elpicode}
...
Since \Ho unification variables have a scope, their solution needs to be
abstracted over these in order to enables the instantiation of a single
...
\noindent
One can see \elpiIn{abs} as a syntax for the lambda abstraction of the meta
language.
%Luckily  we need to reason on these abstractions only when we
%manipulate the solution given by the unification of the meta language; when
%we manipulate the terms we don't 
%
...
The memory \Fo{} is called \elpiIn{fsubst} while the one of \Ho is
called \elpiIn{subst}.
