...
\newcommand{\printAll}[4]{
  ...
  \begin{description}[leftmargin=!,labelwidth=\widthof{\bfseries Mapping:}]
    \printEntry{#1}%
    \printItemComp{Compil:}{\mathbb{T}}{#2}%
...
unification problem between terms $\mathbb{S}_{p_l}$ and
$\mathbb{S}_{p_r}$ taken from the set of all terms $\mathbb{S}$.
...
$$
\begin{array}{l}
\stepF{\mathbb{S}}{p}{\rho}{\rho''}
\deff
\stepFD{\mathbb{S}}{p}{\rho}{\rho'}{\rho''}\vspace{2pt}\\
\runF{\mathbb{S}}{\mathcal{N}}{\rho}
\deff
\runFD{\mathbb{S}}{\mathcal{N}}
\end{array}
$$
...
$$
\begin{array}{l}
\stepH{\mathbb{T}}{p}{\sigma}{\linkStore}{\sigma''}{\linkStore'} \deff\vspace{2pt}\\
  \qquad\stepHD{\mathbb{T}}{p}{\sigma}{\sigma'}{\sigma''}{\linkStore}\vspace{2pt}\\
\runH{\mathbb{S}}{\mathcal{N}}{\rho} \deff \vspace{2pt}\\
  \qquad \mathbb{T} \times \mapStore \times \linkStore_0 = \{ (t_j,m_j,l_j) | s_j \in \mathbb{S}, \C{s_j}{t_j}{m_j}{l_j} \}\vspace{2pt}\\
  \qquad \runHD{\mathbb{T}}{\mathcal{N}}{\linkStore}\vspace{2pt}\\
...
$\forall \mathbb{S}, \forall \mathcal{N},$
$$
  \runF{\mathbb{S}}{\mathcal{N}}{\rho}
  \Leftrightarrow
  \runH{\mathbb{S}}{\mathcal{N}}{\rho}
$$
\end{proposition}
...
In the context of~ \hrun, if $~\mathbb{T} \subseteq \llambda$ we have that
$\forall p \in 1 \ldots \mathcal{N},$
$$
\stepF{\mathbb{S}}{p}{\rho_{p-1}}{\rho_p}
\Leftrightarrow
\stepH{\mathbb{T}}{p}{\sigma_{p-1}}{\linkStore}{\sigma_p}{\_}
$$
\end{proposition}
\noindent
...
$\forall \mathbb{T}, \forall \linkStore, \forall p, \forall \sigma, \forall \sigma'$
$$
\begin{array}{l}
\wellb(\sigma\mathbb{T}) \land
  \sigma\mathbb{T}_{p_l} \Ue \sigma\mathbb{T}_{p_r} \mapsto {\sigma'}
  \Rightarrow \wellb(\sigma' \mathbb{T})\\
\wellb(\sigma\mathbb{T}) \land
...
\cref{sec:compilation}, the unification problem $\mathbb{P}_1$ given 
above, is compiled into the unification problem $\mathbb{T}_1$. 
We can note that all the terms appearing in $\mathbb{P}_1$ are
unifiable by \Uo, but, despite this, \Ue, is not able to solve $\mathbb{T}_1$:
...
The third example shows when a variable occurs outside the scope of a unification
variable cannot be erased and hence prevent a term from being an $\eta$-expansion.\\
...
is in \maybeeta itself, that means it could $\eta$-contract to
...
\begin{lemma}[\maybeeta detection]\label{lem:maybeeta}
  A $\beta$-normal term
  $\lambda x_1 \ldots x_n.t \in \maybeeta$ (as per \cref{def:maybeeta})
  % where each $x_i$ occurs in $t$ is a
  %\maybeeta if
  %either: 
...
there exists a term $t \in \maybeeta$
such that forall substitution $\rho$, $\rho t$ is not an
$\eta$-expansion. A simple counter example is
$\lambda x. f \appsep (A \appsep x) \appsep (A \appsep x)$
which is in \maybeeta{} since $x$ does not \occurrigid{} in the first argument
of $f$,
and the second argument of $f$ \reduceto{} $x$. In other words $A \appsep x$
may either use or discard $x$, but our analysis does not know that \emph{the same
term} cannot have two contrasting behaviors.\\
This is however not a problem in the sense that it does not break
\cref{prop:simulation}. Terms in \maybeeta{} are compiled to
unification variables and a link (see \cref{sec:etacomp}):
the link makes progress (see \cref{sec:etaprogress})
in the same step in which the
variable is instantiated, and that compensates for this coarse analysis.
...
$$
\begin{array}{l}
  \mathbb{P} = \{~ \lambda x.X \appsep x \Uo f ~\}\\
  \mathbb{T} = \{~ A \Ue f ~\}\\
  \mathbb{M} = \{~ \mapping{X}{B}{1} ~\}\\
  \linkStore \;= \{~ \linketaM{}{A}{\lambda x.B_{x ~\}}}
\end{array}
$$
...
$t$ of $\mathbb{P}_1$ has been detected as a \maybeeta and replaced with the
...
$\mathbb{T}_1$, $A$ is assigned to $f$. Therefore \lhs of $l_1$ is a term with
...
Another, way to progress \linketa, that we call \linketa deduplication, is when
...
\printAll
  {{\elpiIn{flam x\ fapp [fuva X, x] == flam x\ fapp [fuva X, x]}}}
  {{E0 == E2}}
  {{\mapping{X}{X'}{1}}}
  {{\linketaM{}{E2}{\lambda x.X'_{x}},
    \linketaM{}{E0}{\lambda x.X'_{x}}}}
...
$$
{\arraycolsep=2pt \begin{array}{rrclrcl}
    \mathbb{P} = \{ &
       \lambda x.\lambda y.X \appsep y \appsep x & \Uo & \lambda x.\lambda y. x & \quad
       \lambda x. f \appsep (X \appsep x) \appsep x & \Uo&  Y ~\}\\
    \mathbb{T} = \{ &
       A &\Ue& \lambda x.\lambda y.x & \quad
       D &\Ue& F ~\}\\
    \mathbb{M} = \{ & \multicolumn{6}{l}{
      \mapping{Y}{F}{0} \quad
      \mapping{X}{C}{2} ~\}}\\
    \linkStore \;= \Big\{  &
      \multicolumn{6}{l}{ \begin{array}{ll}
          \linketaM{}{A}{\lambda x.B_{x}} & \quad
          \linketaM{x}{B_{x}}{\lambda y.C_{y x}}\\
          \linketaM{}{D}{\lambda x.(f~E_{x}~x)} & \quad
          \linketaM{y}{E_{y}}{\lambda w.C_{y w}}
      \end{array} ~\Big\}}
\end{array}}
$$
\marginpar{aggiustare i nomi delle variabili nel testo}
