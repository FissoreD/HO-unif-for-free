...
{\arraycolsep=2pt  
  \begin{array}{rrclrcl}
    \mathcal{P} = \{ & \lambda x.\lambda y.X \appsep y \appsep x & \Uo & \lambda x.\lambda y. x &
         \quad\lambda x. f \appsep (X \appsep x) \appsep x & \Uo&  Y ~\}\\
    \mathcal{T} = \{ & A &\Ue& \lambda x.\lambda y.x & \quad D &\Ue& F ~\}\\
    \mathbb{M} = \{ & \multicolumn{6}{l}{\mapping{Y}{F}{0} \quad \mapping{X}{C}{2} ~\}}\\
    \linkStore \;= \Big\{  & \multicolumn{6}{l}{
  \begin{array}{ll}
    \linketaM{}{A}{\lambda x.B_{x}} &
    \quad\linketaM{x}{B_{x}}{\lambda y.C_{y x}}\\
    \linketaM{}{D}{\lambda x.(f~E_{x}~x)} &
    \quad\linketaM{y}{E_{y}}{\lambda w.C_{y w}}
  \end{array}
  ~\Big\}}
\end{array}}
$$
