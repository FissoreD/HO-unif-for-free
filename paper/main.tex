...
does not contradict \Eo. Also note that a compiler that
...
\begin{proposition}[Compilation round trip]\label{prop:comprt}
  ...
  % if \elpiIn{T} $\in$ S
  If $\C{s}{t}{m}{l}$, such that $l \in \linkStore$ and $m \in \mapStore$, if
  $\sigma = \{ A \mapsto t\}$ and $\mapping{X}{A}{n} \in \mapStore$
  then $\D{\sigma}{\mapStore}{\linkStore}{\rho}$ and
  $\rho X \Eo \rho s$.
\end{proposition}
\todo{explain no proof, llam link breaks}
...
\begin{lemma}
  Given a \linketa \linketaM{\Gamma}{X}{T}, the unification done by \progressetaleft is between
  terms in $\wellb$  
...
\begin{proof}[Proof sketch]
  ...
  By\todo{reformulate}
  \progressetaleft, if 1) the $\sigma X$ is a name, a constant or an
  application, then, $\lambda x.X \appsep x$ is unified with the \rhs. By
  \cref{inv:link-eta-right} and \cref{lemma:unif-eta-aux},
  $\rhs = \lambda x. t$ and $\wellb(t)$.
  Otherwise, 2) $\sigma X = \lambda x.t$.
