...
a \Fo{} language (for first order) with a richer equational
...
Note that $\sigma'$ is a refined (i.e. extended) version of $\sigma$.
...
% \todo{dire che P e T sono liste di coppie, dire che P e T vengono anche usati come insiemi}
We represent a logic program \emph{run} in \Fo{} as
a list \emph{steps} of length $\mathcal{N}$.
\foUnifPb is a list of pair of terms, each pair correspond to a unification
problem. At each step $p$ we unify two terms $\foUnifPb_{p_l}$ and
$\foUnifPb_{p_r}$ (the small $l$ and $r$ represent respectively the left and right 
elements extracted from $P_p$).
...
each pair of \Fo{} terms in \foUnifPb into a pair of \Ho{} terms $t$.
...
for the pair $(\hoUnifPb_{p_l}, \hoUnifPb_{p_r})$.\\
...
\todo{say that this is runtime, while the other fidelity is compile time}
...
\noindent
\todo{dire perchè W(T), dire che il converse non vale (alcuni check li fa la progress)}
Note that the property holds for any substitution, it could be given by an
oracle and not necessarily a most general one, and for any terms,
in particular terms in $\maybebeta{} ~\cup~ \maybeeta{} ~\cup~ \notllambda{}$.
...
they have the same type in the meta-level and the number of \elpiIn{abs}
nodes in the assignment matches that length. In turn this grants that
\elpiIn{move} never fails.\todo{convoluted}
...
the \elpiIn{inctx} container (see rule~\ref{data:inctx}).
...
Since links are of the form uvar = term (\fullref{inv:linklhs})
...
\todo[inline]{dire che però si passa per una subst in cui ste abs
le cambio in lam. Nel codice Coq ci scrivevamo il tipo nella
arity, e quindi sappiamo fare i lambda bene, senza perdita di
informazione. Qui i lam non hanno info, facile. Ma in generale
bisogna spiegare come ci si salva. Ci dormo su: o non generiamo
la subst ma solo il primo termine (la query iniziale) istanziato
(funziona sempre, la prova è quella sopra) oppure bisogna siegare
tutto sto casino e serve un po' di spazio.}

%\input{code/decompile_type}
...
\begin{proof}[Proof sketch]
  By the definition of \mapdeduplication, any two same \Fo{} variables $X_1, X_2$
...
\newcommand{\progBetaLL}{\emph{progress-\llambda-refine}\xspace}
\newcommand{\progBetaRH}{\emph{progress-\llambda-\rhs}\xspace}
% \newcommand{\progBetaDedup}{\emph{progress-\llambda-dedup}\xspace}
\newcommand{\progBetaFail}{\emph{progress-\llambda-fail}\xspace}

Given a \linkbeta $l$ of the form \linkbetaM{\Gamma}{T}{\rhsBeta}, we provide
$4$ different activation rules:
...
\begin{theorem}[Fidelity with \linkbeta]
  The introduction of \linkbeta guarantees \fullref{prop:fidelity}
\end{theorem}
...
In this paper we show how to lift the meta language higher-order unification
procedure to the object language. Our proposed approach is highly adaptable to
...
Our encoding leverages the advantage of not needing to recode the unification
algorithm of the object language. Instead, it utilizes the unification of the
meta language facilitated by the various links we establish to handle
``problematic'' subterms. Additionally, our encoding benefits from the
application of indexing algorithms for static clause filtering.
