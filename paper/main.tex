...
``problematic'' subterm $tp$ is replaced by a fresh unification variable $h$
and an accessory link that represent a suspended unification problem
$h \Ue tp$. As a result \Ue is ``well behaved'' on $t$, meaning it does not
...
\Rightarrow \wellb((\sigma \cup \sigma')\mathcal{T})
$$
\end{proposition}
