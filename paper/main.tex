...
In the case of the OL unification variables \elpiIn{fuv} have no scope.
For example the statement of the instance \coqIn{forall_dec} contains the
term \coqIn{P x} where \coqIn{P} stands for a function of type
\coqIn{A ~$\to$~ Prop}. However our representation of \coqIn{P x} in the meta
language is \elpiIn{app[P, x]} where \elpiIn{P} is of type \elpiIn{ftm}.
In the terms of the ML unification variables come with a scope (the second
argument of the \elpiIn{uva} constructor).
Of course some attention
should be payed when dealing with \elpiIn{uva}, since we have to certify
that an \elpiIn{uva i} remain in the pattern fragment, that is, the
...
\marginpar{maybe move code to the appendix}
\begin{elpicode}
...
\cref{sec:encodings}, <<\elpiIn{x\ f x}>> has not equal to 
...
\noindent
In particular, the node \elpiIn{abs} stands for the lambda abstraction of the ML.
The
...
The memory of the ML is represented by a \elpiIn{subst}.
...
fderef S (fapp [fuva N|A]) (fapp [fuva N|B]) :- unset N S,
  forall2 (fderef S) A B.
fderef S (fapp A) (fapp B) :- forall2 (fderef S) A B.
...
\marginpar{what about app uva?}
\begin{elpicode}
  type deref subst -> tm -> tm -> o.
  % ... the same
  deref S (uva N A) R :- set N S T,
    instantiate T A T', deref S T' R.
  deref S (uva N A) (uva N B) :-unset N S,
    forall2 (deref S) A B.
\end{elpicode}


In particular, 1) if the variable \elpiIn{fuva N} is assigned to a term
...
variable \elpiIn{fuva N} assigned to a list of terms \elpiIn{L} and \elpiIn{uv N}
...
\begin{figure}
  \begin{elpicode}
    type ~(\Eo)~ ftm -> ftm -> o.
    % congruence
    fapp A ~\Eo~fapp B :- forall2 ~(\Eo)~ A B.
...
\begin{figure}
\begin{elpicode}
type ~(\Ue)~ tm -> tm -> subst -> subst ->  o.
% Congruence
(app A ~\Ue~app B) S S1 :- fold2 ~\Ue~A B S S1.
(lam F ~\Ue~lam G) S S1 :- 
  pi x\ (pi S\ (x ~\Ue~x) S S) => (F x ~\Ue~G x) S S1.
~\Ue~(con C) (con C) S S.
% deref
(uva N A ~\Ue~T) S S1 :- set N S F,
  beta-abs F A T1, (T1 ~\Ue~T) S S1.
(T ~\Ue~uva N A) S S1 :- set N S F,
  beta-abs F A T1, (T ~\Ue~T1) S S1.
% flex-flex
(uva N A ~\Ue~uva M B) S S3 :-  unset M, unset N,
  pattern-fragment A, pattern-fragment B, 
  new S W S1, prune W Args1 B Ass, 
  assign N S1 Ass S2, assign M S2 Ass S3.
% assignment
(uva N A ~\Ue~T) S S1 :- pattern-fragment A,
  not (T = uva _ _), not_occ N S T,  
  bind A T T1, assign N S T1 S1.
(T ~\Ue~uva N A) S S1 :- pattern-fragment A,
  not (T = uva _ _), not_occ N S T, 
    bind A T T1, assign N S T1 S1.
%  Note: (a infix b) c d === (infix) a b c d

\end{elpicode}
\caption{The \Ue predicate // TODO move to the appendix}
\label{code:ue-pred}
