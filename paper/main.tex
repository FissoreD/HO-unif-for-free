...
\deff
\stepFD{\mathcal{S}}{p}{\rho}{\rho'}{\rho'}\\
...
\runH{\mathcal{S}}{\mathcal{N}}{\rho} \deff \\
  ...
  \qquad \runHD{\mathcal{T}}{\mathcal{N}}{\mathbb{L}}\\
...
\end{proposition}
\noindent
Note that this property guarantees that a failure in the \Fo run
is matched by a failure
in \Ho
\emph{at the same step}.

\newcommand{\maybebeta}{\ensuremath{\beta}\xspace}
\begin{definition}[\maybebeta]
  $t$ is in \maybebeta if it contains a subterm matching the
  pattern \elpiIn{app[uva _ _|_]}
\end{definition}

\newcommand{\maybeeta}{\ensuremath{\eta}\xspace}
...
$\mathcal{S} = \{ s_1, s_2 \}$ we can define \Uo as follows:
$$
\begin{array}{l}
s_1 \Uo s_2 \mapsto \rho \deff\\
\quad\C{s_1}{t_1}{m_1}{l_1} \land \C{s_2}{t_2}{m_2}{l_2}\\
...
decompilation can introduce terms in \maybebeta or \maybebeta. Nevertheless
\Uo has the following properties (where we
write $s \in \llambda$ if all unif variables in
...
\begin{itemize}
\item \elpiIn{app[F,con "a"]} may be \Eo to \elpiIn{con "a"}
\item \elpiIn{lam x\lam y\app[F,y,x]} may be \Eo to \elpiIn{con "f"} 
\end{itemize}

\noindent
\solve{} solves links when they are no more ``problematic'', for example
in the examples above if \elpiIn{F} materializes, or if the
