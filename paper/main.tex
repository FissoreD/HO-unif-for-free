...
\begin{abstract}
  ...
  In this paper we propose to translate \Fo{} unification problems
  into an alternative encoding called \Ho, from which we derive \Uo,
  the higher-order unification procedure of \Fo{}. \Uo
  honours $\eta\beta$-equivalence for terms
  within the pattern fragment, and allows the use for 
  heuristics when unification is performed with terms outside the pattern fragment.
...
Similarly to \cite{pfenning1993}, a constraint is a delayed unification problems
suspended on a list of variables and is resumed \emph{as soon as one of these variable
is assigned}, before any other existing goal is considered. We point out that
we extend \cite{pfenning1993}, since we delay not only \notllambda unification problems, 
but also those problems that are still in \maybeeta.  
