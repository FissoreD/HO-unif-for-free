...
substitution can be taken for granted when ML binders represent object
logic ones. The second one that comes to mind is unification, the cornerstone
for proof construction and proof search, however in this paper we describe how
reusing that brick may not be as easy at is seems.
...
In some cases, the most notable one being Higher Order Logic~\cite{books/sp/NipkowPW02},
the ML Isabelle is such a good fit that it implements an interactive proof
system for HOL, and not just a specification.

The object logic we are interested in is Coq's~\cite{Coq-refman}
Calculus of Inductive Constructions (CIC), and we want to implement a
type-class~\cite{wadler89} solver for Coq~\cite{Coq-refman} using
the Coq-Elpi~\cite{tassi:hal-01637063} meta programming language,
a dialect of $\lambda$Prolog already used to extend
Coq~\cite{tassi:hal-01637063,tassi:hal-01897468,gregoire:hal-03800154,newtc}.
Type-class solvers are unification based proof search procedures
that  provide essential automation to widely used Coq libraries.
These solvers are reminiscent of Prolog: they back-chain lemmas taken
from a designated database of type class instances.

As an example we take the \coqIn{Decision} type class
from the Stdpp~\cite{JUNG_KREBBERS_JOURDAN_BIZJAK_BIRKEDAL_DREYER_2018}
library. The class
identifies predicates equipped with a decision procedure.
The following three designated \coqIn{Instances} state that:
1) the type \coqIn{fin n}, of natural numbers
smaller than \coqIn{n}, is \coqIn{Finite} (another type class);
2) the predicate \coqIn{nfact n nf}, linking a natural number
\coqIn{n} to its prime factors \coqIn{nf}, is decidable;
...
\noindent Given this database of instances a type-class solver is
able to prove the following statement automatically,
by back-chaining the lemmas above:

\begin{coqcode}
  Check _ : Decision (forall n: fin 7, nfact n 3).       ~\customlabel{goal:g}{(g)}~
\end{coqcode}

\noindent
The encoding of CIC provided by Elpi, that we will discuss at length later in
\cref{sec:encodings,sec:lang-spec}, is an Higher Order Abstract
...
\noindent
After unifying the head of rule \ref{clause:r3a} with the goal, Elpi would run
the premise <<\elpiIn{decomp Pm A P}>> in charge of bringing the
...
\noteE[inline]{extend \Eo and \Ee with reflexivity on uvars.}
...
compiled term. These links are checked for or progress \noteE[inline]{XXX improve....}
...
\noteE[inline]{fix}Property \ref{prop:simulation} states that \Uo, hence our compilation scheme,
...
Our encoding of CIC may look ``semi shallow'' since we use the meta-language
...
In the literature we could find a few related encoding of CIC.
TODO In~\cite{felty93lics} is related and make the
discrepancy between the types of ML and CIC visible. In this case
...
\noteD{add ref to \cref{sec:invariant1}}
...
\elpiIn{commit-link} predicate. In particular, given \linketaM{\Gamma}{X}{t}, we
...
remove from \linkStore and \elpiIn{commit-links} terminates.
...
\begin{proof}[Proof sketch]
  ...
  has one lambda, therefore $\wellb(t)$. Otherwise, 2) \lhs has \elpiIn{lam} as
...
\begin{lemma}
  \Cref{prop:nf} holds, i.e., given a substitution $\sigma$ and a \linketa $l$,
...
\noteD[inline]{Proove simulation fidelity, dicendo che \progressetaright è inutile}
...
terms $t$ in \maybebeta and replace them with fresh variables $X$.
...
\newcommand{\progBetaDedup}{\emph{progress-beta-dedup}}
...
\begin{definition}[\progBetaDedup]
  Given two \linkbeta $l1$ and $l2$, if they share the same \lhs, unification
  fails.
...
\begin{lemma}
  Given a \hoUnifPb and a substitution $\sigma$ then the resolution of
  $\sigma\hoUnifPb$ guarantees \cref{prop:fidelity} 
\end{lemma}

\begin{proof}[Proof sketch]
  If $\sigma\hoUnifPb$ is in \llambda, then by
  \cref{def:progBetaLL,def:progBetaRH}, then \linkbeta disappear and the
  unification done between terms in \llambda. This problem unifies iff
  its corresponding \Fo{} problem unifies too. If $\sigma\hoUnifPb$ is in \maybebeta,
...
\subsection{Relaxing \cref{lemma:keep-fidelity} : tobe renamed}

\newcommand{\progBetaNoLLWait}{\emph{progress-beta-\maybebeta}}
...
delay the resolution of \maybebeta unification problems if the substitution
...
In the example above, $\foUnifPb_1$ is in \maybebeta and the object language
...
modification is quite simple to manage: we are introducing a new \maybebeta
...
A second strategy to deal with problem that are in \maybebeta is to make some
...
\maybebeta. For instance, in $X \appsep a \appsep b = Y \appsep b$, the last
...
The \elpiIn{commit-link} predicate can be extended to add heuristics if during
...
\section{Actual implementation in Elpi}

In this paper we show a minimized example. The full code is there.
But we also have to code things in Coq-Elpi.

In Elpi we don't have a main loop, we rely on the interpreter one.
So links are constraints and progress are CHR rules.
Constraints are suspended goals that are resumend when some unif variable
is assigned.

\begin{elpicode}
link-eta bla :- suspend-condition Holes, !, declare_constraint (link-eta bla) Holes.
link-eta bla :- progress. % solves the goal, hence the constraint is not more in L.
\end{elpicode}

This matches with all progress rules but for the ones considering
two links. For these we need to resort to CHR to manipulate L. For example
deduplicate eta 

\begin{elpicode}
constraints link-eta {
