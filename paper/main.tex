...
\begin{abstract}
  ...
  to related problems in \Ho.
    As a result we can lift the ML
  higher-order unification procedure to \Fo{} in a way that it
  honours $\eta\beta$-equivalence (for CIC functions), solves
  problems in the pattern fragment and finally allows
  for the addition of heuristics to deal with problems outside the
  pattern fragment. The unification procedure we build can be used to
  efficiently simulate a logic program in \Fo{} by re-using optimizations
  provided by the ML such as clause indexing.
...
In this paper we show how to lift the meta language higher-order
unification procedure to the object language. The resulting
unification honours $\eta\beta$-equivalence for 
