...
Even if we encountered the problem in CIC one does not need such a
complex system in order to state it. The minimal setting we could find
is one where a \Fo{} language (for first order) with a rich(er) equational
theory is phrased in a \Ho{} meta language with a simpler one, and one
needs to build the unification of \Fo{} on top of the one of \Ho{}.
...
\subsection{eq and unif}

In order to specify the two unification procedures we need to
define the equational theories of \Fo{} and \Ho{} and define
...
\noindent
Note that move strongly relies on invariant \ref{inv:uvaarity}
that we shall define later.\todo{fixup}
...
\subsection{Logic program simulation}
...
it does not contradict \Eo as it would otherwise do on ``problematic'' terms.
...
\section{Preliminaries for \Fo{} and \Ho}
\label{sec:lang-spec}
\todo{maybe move here napp beta deref fderef move}
