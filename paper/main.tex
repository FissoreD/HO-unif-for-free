...
\marginpar{the compiler must do beta normal!!!!}
...
\begin{enumerate}
  ...
  \marginpar{do we have the name case in the code?}
\end{enumerate}
\end{definition}
...
the context $\Gamma$).
\marginpar{there was a note about T' having a rigid head, TO BE FIXED}
\end{definition}
...
\paragraph{Example of \progressetaright}\marginpar{TODO}
...
\marginpar{I don't get this TODO}
...
\printAlll
  ...
  {{{\eta,x,E_{x},\lambda y.C_{x y}},
    ...
    {\eta,,A,\lambda x.B_{x}},
    {\eta,x,B_{x},\lambda y.C_{y x}}}}
\marginpar{TODO: non so se serva spiegare l'esecuzione (in commento)}
...
in \maybebeta. Unification in \maybebeta is in general a non-decidable
procedure, e.g. $X \appsep a \Uo a$ is a unification problem in \maybebeta,
since we have $X a$ has the shape $X t_1\dots t_n$ where $X$ is a unification
variable and $t_1\dots t_n$ is not a list of distinct names (in our example, $a$
is a constant, hence not a name). We also point out that this unification
problem admits two different substitutions: $\rho_1 = \{X \mapsto \lambda x.x\}$
and $\rho_2 = \{X \mapsto \lambda \_.a\}$.

It is the case, however, given a list of unification $n$ problems \foUnifPb, if
$\foUnifPb_i$ is in \maybebeta, it is possible that the resolution of the
problems $\bigwedge_{j=0}^{i-1}\foUnifPb_j$ gives a partial substitution
$\rho$, such that $\rho\foUnifPb_i$ falls again in \llambda.
%
...
$\foUnifPb_2$, can be solved in \llambda. On the other hand, we see that, 
...
In order to encompass this unification, term compilation should capture the
terms $t$ in \maybebeta and replace them with fresh varaibles $v$. As per
...
\begin{invariant}[\linkbeta \rhs]
  ...
  flexible variable and $t_1\dots t_n$ is not in \llambda.
\end{invariant}
...
\begin{corollary}
  ...
  counterpart is still in \maybebeta, the current unification problem is not in
  \llambda and the unification fails.
\end{corollary}
...
scope all the free variables occurring in \elpiIn{Pf1}.

\newcommand{\rhsBeta}{\ensuremath{N_S\appsep L}\xspace}

\begin{invariant}
  The \rhs of a \linkbeta has the shape \rhsBeta (equivalent to
  \elpiIn{app[uva N S|L]}).
\end{invariant}

\begin{corollary}
  Let \elpiIn{app[uva N S|L]} be the \rhs of a \linkbeta, then \elpiIn{L} is not
  empty.
\end{corollary}

\begin{proof}[Proof sketch]
  By contradiction, if \elpiIn{L} is a empty list, then the original \Fo{} term
  has the shape \elpiIn{fapp[fuva M | Ag]} where \elpiIn{Ag} is a list of
  distinct names (i.e. there list \elpiIn{Extra} is empty). This case is however
  captured by rule~\ref{rule:complam} (from \cref{sec:compilation}) and no
  \linkbeta is produced: a contradiction.
\end{proof}

\begin{corollary}
  Let \rhsBeta be the \rhs of a \linkbeta, then the 
  first argument $t$ in \elpiIn{L} either appears in \elpiIn{S} or it is 
  not a name.
\end{corollary}

\begin{proof}[Proof sketch]
  By construction, the lists \elpiIn{S} and \elpiIn{L} are built by splitting
  the list \elpiIn{Ag} from the original term \elpiIn{fapp [fuva A|Ag]}.
  \elpiIn{S} is the longest prefix of the compiled terms in \elpiIn{Ag} which is
  in \llambda. Therefore, by definition of \llambda, either the first element of
  \elpiIn{L} is a name appearing in \elpiIn{S} or it a term with a constructor
  of \elpiIn{tm} as functor.  
\end{proof}

TODO: Dire che maybe eta fa il detect anche su 
termini che non sono il llambda, oppure dirlo in section of maybeeta + dare un
esempio?

\subsection{Progress}

\newcommand{\progBetaLL}{progress-beta-llambda}
\newcommand{\progBetaRH}{progress-beta-rigid-head}
...
\begin{definition}[\progBetaLL]
  A link \linkbetaM{\Gamma}{X}{N_S\appsep [t|L]} is removed from \linkStore, if
  it exists a substitution $\gamma$, such that the first term $t$ is a name no
  occurring in $S$. In this case, let $M$ a fresh variable, then the new link
  \linkbetaM{\Gamma,x}{X}{M_{S,n}\appsep L} is added to \linkStore and if $L =
  []$, then $X$ is unified with $M_{S,n}$, otherwise, the
  \linketaM{\Gamma}{N}{\lambda x.M_x} is added to \linkStore.
  \label{def:progBetaLL}
\end{definition}

\begin{definition}[\progBetaRH]
  A link \linkbetaM{\Gamma}{X}{\rhsBeta} is removed from
  \linkStore if $N_S$ is instantiated to a term $t$ and the
  $\beta$-reduced term $t'$ obtained from the application of $t$ to
  $L$ is in \llambda. Moreover, $X$ is unified to $t$.
...
\begin{proof}[Proof sketch]
  By \cref{def:progBetaRH}, the \linkbeta is removed from \linkStore, hence 
  they cannot be applied indefinitely. If the \linkbeta is progressed due to
  \progBetaLL, then two links are added, the first 
\end{proof}

\begin{corollary}
  Given a \linkbeta, the scope of its \rhs variables in \llambda.
\end{corollary}

\begin{proof}[Proof sketch]
  By construction, the \rhs of \linkbeta is of the form \rhsBeta and $S$ is in
  \llambda. If a \linkbeta is triggered by \progBetaRH, then, by
  \cref{def:progBetaRH} that link is removed by \linkStore, and the property is
  satisfied. If the \linketa is activated by \progBetaLL, then, by
  \cref{def:progBetaLL}, $L = [n|L']$ and the scope `$S,x$', is in \llambda. 
\end{proof}
