...
we modify the term compiler by putting fresh unification variables
in their place: the problematic term is moved from 
...
it can only be an $\eta$-expansion of it, or a unification
variable that can be assigned to it, or a combination of both.
...
\begin{definition}[\testmaybeeta]\label{def:testmaybeeta}
  ...
  \begin{enumerate}
    \item $t$ is a constant or variable applied to arguments
...
$\foUnifPb_2$, can be solved in \llambda. On the other hand, even though \Uo is
able to solve \foUnifPb, \Ue can't solve the compiled problems \hoUnifPb. In
...
not unifable problem $(\lambda x. B) \appsep a \Ue a$.

In order to encompass this unification, we capture terms $t$ in \maybebeta and
replace them with fresh varaibles $v$. As per \maybebeta, the variables $v$ and
the terms $t$ are linked through a \linkbeta.

\linkbeta guarantee \cref{inv:linklhs} and the term on the \rhs has the
following property:
...
\begin{proof}[Proof sketch]
  ...
  is replaced with a variable $V$, and the \linkbeta \linkbetaM{}{V}{X
...
the application of a fresh variable \elpiIn{C} having in scope all the compiled
list of terms obtained from \elpiIn{Pf} and \elpiIn{Extra1} as arguments. The
variable \elpiIn{B} returned has compiled terms is a fresh variable having in
scope all the free variables used in \rhs.

\begin{invariant}
  The \rhs of a \linkbeta has the shape \elpiIn{app[uva N S|L]}.
\end{invariant}
...
\begin{proof}[Proof sketch]
  By contradiction, if \elpiIn{L} is a empty list, then the original \Fo{}
  term has the shape \elpiIn{fapp[fuva M | Ag]} where \elpiIn{Ag} is a list of 
  disting names. This case is however captured by rule~\ref{rule:complam} (from
  \cref{sec:compilation}) and no \linkbeta is produced: a contradiction.
\end{proof}

\begin{corollary}
  Let \elpiIn{app[uva N S|L]} be the \rhs of a \linkbeta, then the 
...
under a given substitution. There are two main situation causing this
progression.

\begin{definition}[progress-beta-llambda]
  
\end{definition}

\begin{definition}[progress-beta-rigid-head]
