...
Finally, the \maybeeta detection defined just before is an
...
If \lhs is instantiated to a rigid term $t$, by \cref{inv:subst-nf},
...
%
\printAll
  {{\elpiIn{flam x\flam y\fapp[fuva F,y,x] = flam x\flam y\x},
    \elpiIn{flam x\fapp[fcon"f", fapp[fuva F, x], x] = fapp[fcon"f", flam x\x]}}}
  {{X = \lambda x.\lambda y.x, Z = f\appsep(\lambda x.x)}}
  {{\mapping{F}{G}{1}, \mapping{F}{H}{2}}}
  {{\linketaM{x}{Y_{x}}{\lambda y.H_{y~x}},
    \linketaM{\phantx}{X}{\lambda x.Y_{x}},
    \linketaM{\phantx}{Z}{\lambda x.f \appsep G_{x}~x}}}
