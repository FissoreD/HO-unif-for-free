...
\begin{invariant}[Unification variable arity] Each variable \elpiIn{A}
in \Ho has a (unique) arity \elpiIn{N} and each occurrence~
\elpiIn{(uvar A L)} is such that \elpiIn{(len L N)} holds
\end{invariant}

\noindent The arity of a variable in \Ho (a \elpiIn{hvariable} is stored in
the mapping. In particular \elpiIn{m-alloc}
bla bla explain. Multiple mappings for the same \elpiIn{fvariable}
are handled in section \ref{sec:eta}.

\input{code/comp_links}

\subsection{Notations}

When we write \Ho terms outside code blocks we prefer to use the
usual and more compact mathematical notation. However we need to
distinguish between the ``application'' of a unification variable
to its scope and the application of a term (possibly a unification
variable) to a list of arguments. We write the scope in subscript
and we use juxtaposition for application. Here a few examples:
\vspace{5pt}
...
\noindent
When detailing example we prefer to write links in mathematical notation,
as equations between terms in a context. The equality sign is subscripted with
...
\subsection{Problems with $\eta$}\label{sec:eta}
...
\subsection{Problems with $\beta$}\label{sec:beta}
...
\elpiIn{Scope2} and \elpiIn{Extra2}.
