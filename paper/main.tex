...
unification problem $\mathbb{P}$ is compiled as follows:
%
\printAlll
...
\noindent
While the original problem should be solved by \Uo{} (by assigning $f$ to $X$)
\Ue{} is not able to solve the problem in $\mathbb{T}$:
its left and right hand sides have different rigid heads:
$\lambda x.A_x$ corresponds to the \Ho{} term
\elpiIn{lam x\ uva A [x]} 
while $f$ corresponds to
\elpiIn{con"f"}. 

In order to guarantee \cref{prop:simulation} we detect
lambdas that can disappear by eta contraction (the \maybeeta set
introduced in \cref{sec:nutshell}); then 
we modify the way terms are compiled and move terms in \maybeeta
from  $\mathbb{T}$ to the \linkStore and put fresh unification variables
in their place.

\elpiIn{type link-eta tm -> tm -> baselink}

\noindent
As per \cref{inv:linklhs} the term on the left hand side
(\lhs) is a variable, and its right counterpart (\rhs) is the
term in \maybeeta and it has the following property:

\begin{invariant}[\linketa \rhs]
  The \rhs of any \linketa in \linkStore has the shape $\lambda x.t$
...
\newcommand{\testmaybeeta}{\emph{maybe-eta}}
...
\noindent
The rule verifies if the \Fo{} term $\lambda t$ in input is in
\maybeeta. Let $\lambda t'$ be the compiled version of $\lambda t$, then the fresh
...
\printAlll
  ...
  {{{Y,F,0},
    {X,E,1},
    {X,C,2}}}
  {{{,D,\lambda x.(f\appsep E_{x}\appsep x)},
    {,A,\lambda x.B_{x}},
    {x,B_{x},\lambda y.C_{y x}}}}
