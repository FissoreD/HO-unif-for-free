...
\mapping{\elpiIn{A}}{\elpiIn{A'}}{1}. However, if the oracle sets \elpiIn{A'} to the constant
...
recognizes \maybeeta subterms and replaces them with fresh \Ho variables. 
This link between the variable $v$ and the subterm $t$ is stored in what we
...
reduce to $x$, the second one is not a \maybeeta since it exists no substitution
for $A_x$ such that $A_x$ reduces to $x$ and $x$ is not free in the subterm $f \appsep x$.
Eq.~\ref{eq:lamlam} is a bit more
...
As a final remark, note that eq.~\ref{eq:lamdisc} is a \maybeeta even if $x$
occurs in the subterm $t' = f \appsep A_x$. It exists in fact a substitution
...
We can now define more formally the two auxiliary relations needed for \maybeeta
detection:
