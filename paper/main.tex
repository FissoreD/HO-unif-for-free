...
\elpiIn{e} is written <<\elpiIn{x\ e}>>, and square brackets delimit a
...
We write \todo{meglio dire che sigma' estende sigma, togliere unione e quindi dominio da sopra}
...
\todo{spiegare llam (first use after intro)}
...
\todo{add notation for problem compilation}
...
\paragraph{Decompilation}
We suppose that the unification of the object language rejects any unification
between two terms where at least one of them is not in \llambda 
(this includes unifications like $X = Y\appsep Z$ and $X\appsep Y = X \appsep Y$).
Therefore, a failure is thrown if any \linkbeta remains in \linkStore at the
...
\begin{definition}[\progBetaFail]
  ...
  \footnote{This is a consequence of what claimed in \linkbeta decompilation}
...
\subsection{Relaxing \fullref{def:progBetaFail}}
...
such as $\lambda$Prolog~\cite{lamProlog} which 
delay the resolution of \notllambda unification problems if the substitution
is not able to put them in \llambda. For example in 
\noteD{Teijus}
%
