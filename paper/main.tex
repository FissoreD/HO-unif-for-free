...
% Variables in the OL and ML are represented with integers
...
understand this definition, let's take the subterm \coqIn{P x} from the instance
forall\_dec. \coqIn{P} has type \coqIn{A ~$\to$~ Prop} and \coqIn{x} is a name
bound to \coqIn{P}. The application of \coqIn{P} to \coqIn{x} at the OL level,
put this name in the scope of the variable \coqIn{P}. However, at the meta
level, the translation of \coqIn{P x} become \elpiIn{app[uv 1, x]}; \elpiIn{x}
is not in the scope of \elpiIn{uv 1}.

% Uvar in ML have scope
...
performed by the \elpiIn{equal} predicate. Therefore, a unification variabile
$i$ equal to another term $t$ if $t$ is a variable with same index $i$, or if
$i$ is assigend to $t'$ in $\theta$ and \elpiIn{equal ~$t'$~ ~$t$~}. Moreover
...
understand \etabeta, but, since the nodes \elpiIn{app} and \elpiIn{lam} do
