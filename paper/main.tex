...
terms, called \elpiIn{tm}, that includes (among others) the following
constructors:
...
the arguments of a higher-order variable are given via the \elpiIn{fapp}
...
% \input{code/fderef}
It is worth examining the code of \elpiIn{deref}, which
...
The only complex part concerns the \elpiIn{abs} node. In our simple setting,
the \elpiIn{flam} node carries no additional information (other than the function body),
...
enforce the terms passed to \hstep are in \wellb,
...
and the proof will be adjusted to cover for thew cases.

\begin{lemma}[Properties of \Uo]\label{prop:uniffow} The following properties hold for \Uo:
\begin{gather}
  \unifcorrectX{\Ee}{\Ue}{t}{\wellb(\{t_1, t_2\})}\\
  \unifcompleteX{\Ee}{\Ue}{t}{\wellb(\{t_1, t_2\})}
\end{gather}
\end{lemma}
\begin{proof}[Proof sketch]
 In this setting \Ee is as strong as \Eo on ground terms.What we have to show is that whenever two different \Fo{}
...
\fullref{prop:fidelity} hold if $\;\wellb{(\foUnifPb)}$.
\end{theorem}
\begin{proof}[Proof sketch]
...
In the following sections we adopt this notation to talk about the compiler's
output: \foUnifPb is the input set of problems that compiled to
...
We index each sub-problem, sub-mapping, sub-link with its position
...
\printAlll
  ...
  {{}}
\todo{fix indentation}
The unification problem $\hoUnifPb_1$ fails while trying to unifying
$A \appsep x$ and $x$, equivalent to \elpiIn{app [uva A [], x]}
versus \elpiIn{x}.
...
\begin{definition}[$\wellb/_\beta$]
$
\wellb/_\beta(X) \Leftrightarrow \forall t \in \subterm{X}, t \not\in (\maybeeta{} ~\cup~ \notllambda{})
$
\end{definition}

\begin{lemma}[Properties of \Uo]\label{prop:uniffowb} The following properties hold for \Uo
  where
  \begin{gather}
    \unifcorrectX{\Ee}{\Ue}{t}{\wellb/_\beta(\{t_1, t_2\})}\\
    \unifcompleteX{\Ee}{\Ue}{t}{\wellb/_\beta(\{t_1, t_2\})}
  \end{gather}
\end{lemma}

  \begin{proof}[Proof sketch]
  If we look at the \Fo{} terms the is one more interesting case,
  namely \elpiIn{fapp[fuva X|W] ~\Uo~s} when \elpiIn{W}
  are distinct names compiled to $\vec{w}$. In this case
  the \Ho problem is $Y_{\vec{w}} \Ue t$ that succeeds with
   $\sigma = \{ Y_{\vec{y}} \mapsto t[\vec{w}/\vec{y}]\}$ that in turn
   is decompiled to $\rho = \{ Y \mapsto \lambda \vec{y}.s[\vec{w}/\vec{y}]\}$.
   Thanks to \ref{clause:beta1}
   $(\lambda \vec{y}.s[\vec{w}/\vec{y}])~\vec{w} \Eo s$.
  \end{proof}
  
\begin{lemma}[$\wellb/_\beta$-enforcement]\label{lem:w-enforcement-maybebeta}
...
\begin{theorem}[Fidelity in $\wellb/_\beta$] \Fullref{prop:simulation} and
\fullref{prop:fidelity} hold if $\;\wellb/_\beta(\foUnifPb)$
\end{theorem}
...
Activation is implemented by extending the \elpiIn{progress1}
...
\noindent
TWhenever \elpiIn{flam F} is detected to be in
...
\elpiIn{lam F1}. The variable is connected to \elpiIn{lam F1} via a \linketa.
Invariant~\fullref{inv:linklhs} holds for this link, moreover:
...
\begin{proof}[Proof sketch]
  ...
  Otherwise, if $\testmaybeeta{}~\lambda y.t$ does not hold, also
  $\testmaybeeta{}~\lambda x.\lambda y.t$ does not hold, contradicting
...
\begin{proof}[Proof sketch]
  ...
  Otherwise, 2) the \lhs has \elpiIn{lam} as functor.
  \todo{what does as a functor mean?}
...
\begin{lemma}
  The introduction of \linketa guarantees \fullref{prop:w-enforcing}
  %\label{lemma:unif-wellb}
\end{lemma}

\begin{proof}[Proof sketch]
  By \cref{def:testmaybeeta}.\todo{shall we move the proof here?}
\end{proof}
...
\begin{proof}[Proof sketch]
  ...
  \todo{Fix: show that 5.7 an 5.8 compensate for missing eta rule}
...
In turn the rule \todo{which rule} runs $\lambda x.f \appsep x \Ue{} \lambda x.B_{x}$,
...
and a there exist a few strategies to go bejond \llambda without 
implementing Huet's algorithm~\ref{REF}.

Some implementations
of $\lambda$Prolog~\cite{lamProlog} 
such as Teyjus~\cite{Nadathur2001} delay the resolution of \notllambda unification problems until the substitution
 put them in \llambda. Other systems,
for example of the unification algorithm of Coq
used in its type-class solver~\cite{sozeau08},
 apply heuristics like preferring projection over mimic, and commit
 to that solution.
...
(waiting for $X$ to be be instantiated by a future unification),
...
partial since we break \fullref{inv:linklhs}. %: there exists \linkbeta with a non-flexible \lhs.

If we want to follow the second strategy and pick an
arbitrary solution. For instance, in $X \appsep a \appsep b = Y \appsep b$, the last
argument of the two terms is the same and unification can succeed by assigning
$X a$ to $Y$. Note
that this is not the only solution, since $\sigma = \{X \mapsto \lambda x.Y \}$
is another valid substitution. Our unification procedure
can be modified to accommodate this behavior by adding a
rule to \elpiIn{progress} that tries to
align the rightmost arguments and unify
the resulting heads whenever \lhs of a \linkbeta is not in \llambda.

Note that delaying unification outside \llambda can leave \linkbeta for the
...
\todo{cita teyjus (1) era 2nd order HO (huet's algorithm), teyjus 2 è llam ma sospende i disagreement pairs fuori da llam}
...
implementing an unification procedure in the meta
language is likely to be significantly slower compared to the built-in one.
...
\noindent
However, this encoding has two big limitations. Firstly, in CIC, it is
not always feasible to adopt it due to the meta language's type system being
too limited to accommodate for that one the object language. E.g. CIC
can typecheck variadic functions, see for example~\ref{CPDT chlipala}. \\
...
Constructions (CC)~\cite{felty93lics}. The goal of that work was to exhibit
