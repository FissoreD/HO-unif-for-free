...
\def\linkStore{\ensuremath{\mathbb{L}}\xspace}
...
\begin{invariant}[Link left hand side]\ref{inv:linklhs}
  The left hand side of a new link
  is a variable.
\end{invariant}

% TODO: io sposteri l'invariante sopra nella maybe-eta, perchè  
% la proprietà è vera solo per i link eta
% per i link beta, questo non è vero se sospendiamo i link
\noindent
...
the \Fo{} terms \elpiIn{lam x\ app[con "f", x]} and \elpiIn{con "f"} since the
rigid constructor \elpiIn{lam} is different from the rigid constructor
\elpiIn{con}. In conclusion, we need to adapt our \elpiIn{comp} relation in
...
\begin{invariant}[\linketa \rhs]
  ...
  where $F \appsep x$ is a term not starting with the \elpiIn{lam} constructor.
...
prevent the termination of \elpiIn{progress}, since, if the link is woken up,
...
\elpiIn{progress} continues to terminate, since, it would mean that the
condition of the \elpiIn{if}-branchement succeds and the \elpiIn{progress}
