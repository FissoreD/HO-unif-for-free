...
\begin{frame}
  ...
  \begin{itemize}
    \item $X\appsep x$ becomes \elpiIn{A x} with mapping $\mapping{X}{A}{1}$
    \item For example, $\lambda y. X\appsep y = \lambda y. f\appsep y\appsep a$
    \item Is compiled into: \elpiIn{fun (w\ A w) = fun (w\ app[f, w, a])}
  ...
  \begin{itemize}
    ...
    \item An example: %
      \printAlllSingle
        ...
        {{{\elpiIn{"f"},\elpiIn{A}}}}
        {{{X,\elpiIn{B},1}}}
        {{{\eta,,\elpiIn{A},\elpiIn{fun (x\ app["f", B x])}}}}
    \item After unification of \elpiIn{A} with \elpiIn{"f"},
          the lhs of the link becomes rigid and
          \elpiIn{fun (x\ app["f", B x])} is unified with \elpiIn{fun (x\ app["f", x])}
...
\begin{frame}[fragile]
  ...
  \begin{itemize}
    ...
    \item Example: 
      ...
      {{{X,\lambda x.a},
        {(X\appsep a),a}}}
      {{{\elpiIn{A},\elpiIn{fun (x\"a")} },
        {\elpiIn{B},\elpiIn{"a"}}}}
      {{{X,\elpiIn{A},0}}}
      {{{\llambda,,\elpiIn{B},\elpiIn{A "a"}}}}
