...
\begin{frame}[fragile]
  ...
  \def\linkrhs{fun\PYG{+w}{ }\PYG{n+nv}{A}\PYG{+w}{ }\PYG{p}{(}\PYG{n+nv}{x\PYGZbs{}}\PYG{+w}{ }app\PYG{k+kd}{[}\PYG{n+nv}{P\PYGZsq{}}\PYG{k+kd}{,}\PYG{+w}{ }x\PYG{k+kd}{]}\PYG{p}{)}}
...
\begin{frame}[fragile]
  ...
  \begin{itemize}
    ...
    \item After unification of \elpiIn{c"f"} with \elpiIn{A},\\
          \elpiIn{fun (x\ app[c"f", x])} is unified with \elpiIn{fun (x\ app[c"f", B x])}\\
          \elpiIn{B} is assigned to \elpiIn{x\x}
...
\begin{frame}[fragile]
  ...
  \begin{itemize}
    ...
    \item After unification of \elpiIn{A} with \elpiIn{fun (x\ c"a")}, \\
        the rhs of the \llambda-link becomes \elpiIn{c"a"}\footnote{after a $\beta$-reduction step},\\
